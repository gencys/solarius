...
char* p_recently_play[10] EWRAM_BSS;
...
void Show_ICON_filename_SD(u32 show_offset, u32 file_select, u32 haveThumbnail) {
   ...
   for (line = 0; line < need_show_folder; line++) {
      ...
      DrawPic((u16*)(gImage_icon_folder /*gImage_icons+0*16*14*2*/), 0, y_offset + line * 14, 16, 14, 1, 0x0000, 1);

      DrawText(pFolder[show_offset + line].filename, char_num, 1 + 16, y_offset + line * 14, name_color, 1);

      if ((haveThumbnail == 1) && (line > 3)) {
      } else {
         char msg[20];
         sprintf(msg, "%s", "DIR");
         DrawText(msg, 0, 221, y_offset + line * 14, name_color, 1);
      }
   }
   ...
   for (line = need_show_folder; line < need_show_folder + need_show_game; line++) {
      ...
      if (!strcasecmp(&(pfilename[strlen8 - 3]), "gba")) {
      ...
      } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                             "esv")) {  // Fixes the bug with esv files looking like watara supervision
         ...
         icon = (u16*)(gImage_icon_ZX);
                     ...
                     &(pfilename[strlen8 - 2]),
                     "o2")) {  // Magnavox Odyssey2 (No emu yet, but I'm eventually going to have a finished one. :D)
         ...
         icon = (u16*)(gImage_icon_pokem);
                             ...
                             "dci")) {  // Visual Memory Unit (No Emu yet, but I will make one at some point)
         icon = (u16*)(gImage_icon_vmu);
      } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                             "vmi")) {  // Visual Memory Unit (No Emu yet, but I will make one at some point)
         icon = (u16*)(gImage_icon_vmu);
         ...
         icon = (u16*)(gImage_icon_mod);
                             ...
                             "cwz")) {  // Unknown Music file, contained in a package for PogoShell 1.2
         icon = (u16*)(gImage_icon_mod);
      ...
      DrawPic(icon, 0, showy, 16, 14, 1, 0x0000, 1);

      DrawText(pFilename_buffer[offset + line - need_show_folder].filename, char_num, 1 + 16, showy, name_color, 1);
      if ((haveThumbnail == 1) && (line > 3)) {
      } else {
         char msg[20];
         Get_file_size(offset + line - need_show_folder, msg);
         DrawText(msg, 0, 208, showy, name_color, 1);
      }
   }
}
...
void Backup_savefile(const char* filename) {
   ...
   strncpy(temp_filename, BACKUP_SAVER, sizeof(temp_filename) - 2);
...
void IWRAM_CODE Refresh_filename(u32 show_offset, u32 file_select, u32 updown, u32 haveThumbnail) {
   ...
   if ((file_select == (need_show_folder - 1)) && (updown == 3)) {
      DrawText(pFolder[show_offset + xx1].filename, char_num1, 1 + 16, showy1, name_color1, 1);
      DrawText(pFilename_buffer[0].filename, char_num2, 1 + 16, showy2, name_color1, 1);

      if (char_num1 == 32) {
         sprintf(msg, "%s", "DIR");
         DrawText(msg, 0, 221, showy1, name_color1, 1);
      }
      if (char_num2 == 32) {
         Get_file_size(0, msg);
         DrawText(msg, 0, 208, showy2, name_color1, 1);
      }
   } else if (file_select < need_show_folder) {
      DrawText(pFolder[show_offset + xx1].filename, char_num1, 1 + 16, showy1, name_color1, 1);
      DrawText(pFolder[show_offset + xx2].filename, char_num2, 1 + 16, showy2, name_color1, 1);
      ...
      if (char_num2 == 32) {
         DrawText(msg, 0, 221, showy2, name_color1, 1);
      }
      ...
      DrawText(pFilename_buffer[offset + xx1 - need_show_folder].filename, char_num1, 1 + 16, showy1, name_color1,
                   1);
      DrawText(pFilename_buffer[offset + xx2 - need_show_folder].filename, char_num2, 1 + 16, showy2, name_color1,
                   1);

      if (char_num1 == 32) {
         Get_file_size(offset + xx1 - need_show_folder, msg);
         DrawText(msg, 0, 208, showy1, name_color1, 1);
      }
      if (char_num2 == 32) {
...
void Show_ICON_filename_NOR(u32 show_offset, u32 file_select) {
   ...
   for (line = 0; line < need_show; line++) {
      ...
      DrawPic((u16*)gImage_icon_nor /*(gImage_icons+2*16*14*2)*/, 0, y_offset + line * 14, 16, 14, 1, 0x0000, 1);

      DrawText(pNorFS[show_offset + line].filename, char_num, 1 + 16, y_offset + line * 14, name_color, 1);
      sprintf(msg, "%4luM", pNorFS[show_offset + line].filesize >> 20);
      DrawText(msg, 0, 208, y_offset + line * 14, name_color, 1);
   }
}
...
void Refresh_filename_NOR(u32 show_offset, u32 file_select, u32 updown) {
   ...
   DrawText(pNorFS[show_offset + xx1].filename, char_num, 1 + 16, showy1, name_color1, 1);
   DrawText(pNorFS[show_offset + xx2].filename, char_num, 1 + 16, showy2, name_color1, 1);
...
void Show_game_num(u32 count, u32 list) {
   ...
   DrawText(msg, 0, 185, 3, gl_color_text, 1);
}
   ...
   if (shift > timeout) {
      ...
      if (namelen > (char_num - 1)) {
         ...
         if (orgtt != tt) {
            ...
            DrawText(msg, char_num - 1, 1 + 16, y_offset + file_select * 14, gl_color_text, 1);
         }
      }
   }
}
...
void Show_MENU_btn() {
   ...
   sprintf(msg, "%s", MESSAGES[L_MENU_BTN]);
   DrawText(msg, 0, 60, 118, gl_color_text, 1);
}
...
void Show_MENU(u32 menu_select, PAGE_NUM page, u32 havecht, u32 Save_num, u32 is_menu, u32 firstgame) {
   ...
   for (line = 0; line < linemax; line++) {
      ...
      if (page == NOR_list)
         DrawText(MENU_NOR[line], 32, 47, y_offset + line * 14, name_color, 1);
      else {
         if (line == 5)  // cheat
         {
            sprintf(msg, "%s(%ld)", MENU_BOOT[line], gl_cheat_count);
            DrawText(msg, 32, 47, y_offset + line * 14, name_color, 1);
         } else {
            DrawText(MENU_BOOT[line], 32, 47, y_offset + line * 14, name_color, 1);
            ...
            if (line == 4)  // save tpye
            {
               ...
               DrawText(msg, 32, 60 + 54, y_offset + line * 14, name_color, 1);
            }
         }
      }
   }
...
void Show_game_name(u32 total, u32 Select) {
   ...
   for (line = 0; line < need_show; line++) {
      ...
      DrawText(msg, 39, X_offset, Y_offset + line * line_x, name_color, 1);
   }
}

u32 get_count(void) {
   return get_file_lines(RECENT_FILE, p_recently_play, 10, 0, 1);
}
...
u32 show_recently_play(void) {
   ...
   DrawText(MESSAGES[L_RECENT], 0, (240 - strlen(MESSAGES[L_RECENT]) * 6) / 2, 4, gl_color_text, 1);  // TITLE
   ...
   if (all_count) {
      ...
      DrawText(MESSAGES[L_NO_RECENT], 0, 1, 20, gl_color_text, 1);
      while (1) {
         VBlankIntrWait();
         VBlankIntrWait();
         scanKeys();
...
void ShowTime(u32 page_num, u32 page_mode) {
   ...
   DrawText(msgtime, 0, 120, 3, gl_color_text, 1);
}
...
u32 IWRAM_CODE LoadEMU2PSRAM(TCHAR* filename, u32 is_EMU) {
   ...
   u32 filesize;
   u32 res;
   // u32 blocknum, blockoffset = MESSAGES[L_FOLDER_ERR]; // why are you setting it to a string pointer? This is never brought up
   // again outside of overwriting it.
   ...
   switch (is_EMU) {
      ...
      default:
         ...
         f_lseek(&gfile, 0x0000);
         ShowbootProgress(MESSAGES[L_GEN_EMU]);
         for (blocknum = 0x0000; blocknum < filesize; blocknum += 0x20000) {
            sprintf(msg, "%luMb", (blocknum) / 0x20000);
            str_len = strlen(msg);
            Clear(0, 130, 240, 15, gl_color_cheat_black, 1);
            DrawText(msg, 0, (240 - str_len * 6) / 2, 160 - 30, 0x7fff, 1);
   ...
   if (res == FR_OK) {
      ...
      DrawText(MESSAGES[L_WRITING], 0, 78, 160 - 15, 0x7fff, 1);
...
/**
 * @brief Loads the game's thumbnail in the read cache.
 *
 * @param pfilename_pic The path to the game for which to load the thumbnail.
...
void SD_list_L_START(u32 show_offset, u32 file_select, u32 folder_total) {
   ...
   DrawText(MESSAGES[L_DEL_FILE], 0, 60, 60, gl_color_text, 1);  // use sure?MESSAGES[L_DEL_FILE]
   DrawText(pFilename_buffer[show_offset + file_select - folder_total].filename, 20, 60, 75, 0x7fff,
                1);                                  // file name
   DrawText(MESSAGES[L_FINAL_WARN], 5, 60, 90, gl_color_text, 1);  // use sure?
   while (1) {
      VBlankIntrWait();
      scanKeys();
...
void Show_error_num(u8 error_num) {
   ...
   switch (error_num) {
      case 0x0:
         sprintf(msg, "%s", MESSAGES[L_FOLDER_ERR]);
         break;
      case 0x1:
         sprintf(msg, "%s", MESSAGES[L_FILE_ERR]);
         break;
      case 0x2:
         sprintf(msg, "%s", MESSAGES[L_BACKUP_ERR]);
         break;
      case 0x3:
         sprintf(msg, "%s", MESSAGES[L_SAVE_ERR]);
         break;
      case 0x4:
         sprintf(msg, "%s", MESSAGES[L_READ_SAVE_ERR]);
         break;
      case 0x5:
         sprintf(msg, "%s", MESSAGES[L_WRITE_SAVE_ERR]);
         break;
      case 0x6:
         sprintf(msg, "%s", MESSAGES[L_RTS_ERR]);
         break;
      default:
         sprintf(msg, "%s", "error?");
         break;
   ...
   DrawText(msg, 0, 90, 2, RGB(31, 00, 00), 1);
   wait_btn();
}
...
u8 Process_savefile(u32 is_EMU, TCHAR* pfilename, u32 gamefilesize, BYTE saveMODE) {
   ...
   {
      ShowbootProgress(MESSAGES[L_CREATE_SAVE]);
...
void Check_save_flag(void) {
   ...
   if (readd == 0x11) {
      ...
      DrawText(MESSAGES[L_COPY_SAVE], 0, 47, 28, gl_color_text, 1);                    // use sure?MESSAGES[L_DEL_FILE]
      DrawText((TCHAR*)SAV_info_buffer, 20, 47, 40, gl_color_text, 1);       // file name
      DrawText((TCHAR*)SAV_info_buffer + 20, 20, 47, 52, gl_color_text, 1);  // file name
      DrawText((TCHAR*)SAV_info_buffer + 40, 20, 47, 64, gl_color_text, 1);  // file name
      // DrawText(gl_formatnor_info,5,60,90,gl_color_text,1);//use sure?

      if (gl_auto_save_sel) {
         DrawText(MESSAGES[L_SAVING], 0, 47, 88, gl_color_text, 1);  // use sure?MESSAGES[L_DEL_FILE]
         ...
         while (1) {
            ...
            u16 keysdown = keysDown();
            if (keysdown & KEY_A) {
               DrawText(MESSAGES[L_SAVING], 0, 60, 88, gl_color_text, 1);  // use sure?MESSAGES[L_DEL_FILE]
...
int main(void) {
   ...
   DrawPic((u16*)gImage_splash, 0, 0, 240, 160, false, 0, true);
   CheckSwitch();

   res = f_mount(&EZcardFs, "", 1);
   if (res != FR_OK)
   {
      DrawText(INIT_ERR, 0, 2, 20, gl_color_cheat_black, true);
      DrawText(POWER_OFF, 0, 2, 33, gl_color_cheat_black, true);
      while (1)
         ;
   }
   else
   {
      DrawText(INIT_OK, 0, 2, 20, gl_color_cheat_black, true);
      DrawText(LOADING, 0, 2, 33, gl_color_cheat_black, true);
   }
   VBlankIntrWait();
   ...
   if (page_num == SD_list)
   {
      ...
      if (res == FR_OK)
      {
         while (1)
         {
            ...
            {
               // Don't get more directories than we can muster
            ...
            {
               // Don't get more files than we can muster
            ...
            // No need to keep going if we already have the maximum of items
      ...
      Sort_folder(folder_total);
      Sort_file(game_total_SD);
   }
   else
   {
      Read_NOR_info();
   ...
   while (1)
   {
      while (1)  // 2
      {
         ...
         if (updata && gl_show_Thumbnail)
         {
            ...
            strlengba = strlen(pfilename_pic);
         ...
         if (updata && gl_show_Thumbnail && is_GBA && (page_num == SD_list))
         {
            if (haveThumbnail)
               ...
               DrawPic((u16*)(gImage_NOTFOUND), 120, 80, 120, 80, 0, 0, 1);  // show pic not found
         }
...
void Boot_NOR_game(u32 show_offset, u32 file_select, u32 key_L) {
   ...
   // DrawText(gl_Loading,0,(240-strlen(gl_Loading)*6)/2,74, gl_color_text,1);
   ...
   ShowbootProgress(MESSAGES[L_CHECK_SAVE]);
   // memcpy(savfilename,pfilename,100);
   ...
   if (pNorFS[show_offset + file_select].have_patch && pNorFS[show_offset + file_select].have_RTS) {
      ShowbootProgress(MESSAGES[L_CHECK_RTS]);
...
u8 FRAM_save_op(u8 OP) {
   ...
   if (OP == 1) {        // load to fram
      if (res == FR_OK)  // have a old save file
      {
         ...
         Bank_Switching(0);
         res = Loadsavefile(savfilename);
         DrawText(MESSAGES[L_SAV_LOADED], 0, 66, 118 - 15, RGB(00, 31, 00), 1);
      } else {
         DrawText(MESSAGES[L_NO_SAV], 0, 66, 118 - 15, RGB(31, 00, 00), 1);
      }
   } else if (OP == 2) {  // bak fram save
      if (res == FR_OK)   // have a old save file
      {
         DrawText(MESSAGES[L_OVERWRITE], 0, 66, 118 - 15, RGB(31, 00, 00), 1);
         while (1) {
            VBlankIntrWait();
            scanKeys();
            u16 keysdown = keysDown();
      ...
      DrawText(MESSAGES[L_SAV_SAVED], 0, 66, 118 - 15, RGB(00, 31, 00), 1);
   }
   wait_btn();
   return 0;
}
      ...
      if (keysdown & KEY_DOWN) {
         ...
         if (MENU_line == 0) {  // boot to NOR.page
            ...
            if (show_offset + file_select + 1 == game_total_NOR) {
               ...
               DrawText(MESSAGES[L_SELECT_LATEST], 0, 66, 118 - 15, gl_color_text, 1);
               wait_btn();
            }
            return 1;
         } else if (MENU_line == 2) {  //
...
load_file:
   ...
   if (MENU_line < 2) {  // work for psram
      if (gamefilesize > 0x2000000) {
         ShowbootProgress(MESSAGES[L_F_OVERFLOW]);
         wait_btn();
         return 0;
      }
   ...
   {
      ShowbootProgress(MESSAGES[L_LOAD_ROM]);
      f_chdir(currentpath);  // return to game folder
      ...
      int bootmode = ((is_EMU > 3) && (is_EMU < 9)) ? ((is_EMU == 6)   ? 2 : (is_EMU == 7) ? 4 : ((is_EMU == 8) ? 5 : 3)) : gl_toggle_reset;
      ...
      switch (MENU_line) {
         ...
         case 1:  // PSRAM BOOT WITH ADDON
            ...
            if ((gl_reset_on == 1) || (gl_rts_on == 1) || (gl_sleep_on == 1) || (gl_cheat_on == 1)) {
               ...
               ShowbootProgress(MESSAGES[L_CHECK_PATCH]);
               ...
               } else                                   //(have_pat==0)
               {
                  ...
                  get_find:
                     ...
                     ShowbootProgress(MESSAGES[L_CREATE_PATCH]);
         ...
         case 2:                   // WRITE TO NOR CLEAN
            ...
            if (res == 0)  // ok
            {
               ...
               DrawText(MESSAGES[L_NOR_FULL], 0, 0, 160 - 15, gl_color_NORFULL, 1);  //"NOR FULL!"
               wait_btn();
               return 1;
            }
            break;
